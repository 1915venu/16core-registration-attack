...
| Metric | 50 UEs | 1000 UEs (20 Pods) | 1000 UEs (10 Pods) | 1000 UEs (10 Pods, 2 AMFs) |
|---|---|---|---|---|
| Pods | 5 | 20 | 10 | 10 |
| UEs/Pod | 10 | 50 | 100 | 100 |
| Pod Sync Window | **58 ms** | 536 ms | **153 ms - 242 ms** | **76 ms - 148 ms** |
| UE Packet Window | **102 ms** | 786 ms | 337 ms | 281 ms - 352 ms |
| Success Rate | **100%** | **~94%** | ~60% | ~70% |
| AMF Bottleneck | None | Compute Limit | **Head-of-Line Queue** | **Head-of-Line Queue** |

> [!NOTE]
> **Architectural Conclusion:** The Open5GS AMF uses a single-threaded "Per-Connection Sequential Processing" model. Funneling massive UE loads through a small number of SCTP connections (e.g., 10 pods) results in deep queues on the SCTP sockets, causing NAS timers to expire before the AMF thread can reach them. Horizontal scaling to *multiple* AMFs only marginally improves the success rate (~70%) because the queues per-connection remain deep. To scale synchronized registration floods past 1000 UEs on Open5GS, the attacker must maximize the number of parallel SCTP connections (Attacker Pods) rather than cramming more UEs into fewer pods.
